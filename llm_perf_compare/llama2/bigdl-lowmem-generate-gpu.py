...
if __name__ == '__main__':
    ...
    with torch.inference_mode():
        ...
        input_ids = model.tokenizer.encode(input1, return_tensors="pt").to('xpu')
        st = time.time()
        ...
        tok_cnt = len(model.tokenizer.encode(output_str))
        ...
        input_ids = model.tokenizer.encode(input2, return_tensors="pt").to('xpu')
        st = time.time()
        output = model.generate(input_ids,
                                max_new_tokens=args.n_predict)
        torch.xpu.synchronize()
